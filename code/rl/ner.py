...
class ner(nn.Module):
  def __init__(self,
    ...
    if pretrained:  # not None
      ...
      word_embedding_np = np.loadtxt('../../dataset/WordEmbed/' + pretrained +
                                     '_embed.txt',
  ...
  def decode_train(self, label_seq, init_dec_hidden, init_dec_cell,
    ...
    for i in range(label_seq_len - 1):
      ...
      if self.attention:
        ...
        # remove the added dim
        dec_hidden_out = dec_hidden_out.view(current_batch_size,
                                             self.hidden_dim)
  ...
  def train(self, shuffle, result_path, do_evaluation, beam_size):
    ...
    initial_epoch = (
    self.checkpoint["epoch"] + 1) if self.load_model_filename else 0
    ...
    for epoch in range(initial_epoch, initial_epoch + self.max_epoch):
      ...
      for batch_idx in batch_idx_list:
        ...
        #
        # Here we use a linear layer to transform the two-directions of the dec_hidden_out's into a single hidden_dim vector, to use as the input of the decoder
        init_dec_hidden = self.enc2dec_hidden(
          torch.cat([enc_hidden_out[0], enc_hidden_out[1]], dim=1))
        init_dec_cell = self.enc2dec_cell(
      ...
      if do_evaluation:
        ...
        val_loss, val_fscore = self.evaluate(self.val_X, self.val_Y, None, None,
                                             "val", None, beam_size)
        test_loss, test_fscore = self.evaluate(self.test_X, self.test_Y, None,
        ...
        epoch, avg_loss, val_loss, test_loss, val_fscore, test_fscore,
        time_end - time_begin))
        output_file.flush()
      else:
        print("epoch", epoch,
      ...
      is_best = False
      checkpoint_filename = os.path.join(result_path,
                                         "ckpt_" + str(epoch) + ".pth")
  ...
  def decode_beam(self, batch_size, seq_len, init_dec_hidden, init_dec_cell,
                  enc_hidden_seq, beam_size):
    ...
    for t in range(1, seq_len):
      ...
      logP_output_beam = torch.stack(logP_out_list, dim=0).permute(1, 0, 2)
      accum_logP_output_beam = torch.stack(accum_logP_out_list, dim=0).permute(
        1, 0, 2)
    ...
    if self.attention:
      ...
      attention_pred_seq = (attention_seq[seq_len - 1][range(batch_size),
                            input_beam, :])[None, :, :]

    logP_pred_seq = (logP_seq[seq_len - 1][range(batch_size), input_beam, :])[
                    None, :, :]
    accum_logP_pred_seq = (accum_logP_seq[seq_len - 1][range(batch_size),
                           input_beam, :])[None, :, :]
    ...
    for t in range(seq_len - 2, -1, -1):
      ...
      if self.attention:
        attention_pred_seq = torch.cat(
          [(attention_seq[t][range(batch_size), input_beam, :])[None, :, :],
           attention_pred_seq], dim=0)

      logP_pred_seq = torch.cat(
        [(logP_seq[t][range(batch_size), input_beam, :])[None, :, :],
         logP_pred_seq], dim=0)
      accum_logP_pred_seq = torch.cat(
        [(accum_logP_seq[t][range(batch_size), input_beam, :])[None, :, :],
         accum_logP_pred_seq], dim=0)
    # End for t
    ...
    accum_logP_pred_seq = accum_logP_pred_seq.view(batch_size * seq_len,
                                                   self.label_size)
  ...
  def evaluate(self, eval_data_X, eval_data_Y, index2word, index2label, suffix,
               result_path, decode_method, beam_size, max_beam_size, agent,
               reward_coef_fscore, reward_coef_beam_size, f_score_index_begin):
    ...
    for batch_idx in range(batch_num):
      ...
      if decode_method == "greedy":
        label_pred_seq, logP_pred_seq, attention_pred_seq = self.decode_greedy(
          current_batch_size, current_sen_len, init_dec_hidden, init_dec_cell,
          enc_hidden_seq)
      elif decode_method == "beam":
        label_pred_seq, accum_logP_pred_seq, logP_pred_seq, attention_pred_seq = self.decode_beam(
          current_batch_size, current_sen_len, init_dec_hidden, init_dec_cell,
          enc_hidden_seq, beam_size)
      elif decode_method == "adaptive":
        # the input argument "beam_size" serves as initial_beam_size here

        label_pred_seq, accum_logP_pred_seq, logP_pred_seq, attention_pred_seq, episode = self.decode_beam_adaptive(
          current_sen_len, init_dec_hidden, init_dec_cell, enc_hidden_seq,
          beam_size, max_beam_size, agent, reward_coef_fscore,
  ...
  def decode_beam_step(self, beam_size_in, y_beam_in, beta_beam_in,
                       dec_hidden_beam_in, dec_cell_beam_in, accum_logP_beam_in,
                       enc_hidden_seq, seq_len, attend_index):
    ...
    for b in range(beam_size_in):
      ...
      if self.attention:
        dec_hidden_out = dec_hidden_out[None, :, :]  # add 1 nominal dim
        dec_hidden_out, attention = \
          self.attention(dec_hidden_out, enc_hidden_seq, attend_index,
                         self.enc2dec_hidden)
  ...
  def decode_beam_adaptive(self, seq_len, init_dec_hidden, init_dec_cell,
                           enc_hidden_seq, initial_beam_size, max_beam_size,
                           agent, reward_coef_fscore, reward_coef_beam_size,
    ...
    cur_state = self.make_state(accum_logP_matrix, logP_matrix, 1,
                                max_beam_size)
    action = None
    ...
    label_pred_seq, accum_logP_pred_seq, logP_pred_seq, attention_pred_seq = self.backtracking(
    ...
    for t in range(1, seq_len):
      ...
      label_pred_seq, accum_logP_pred_seq, logP_pred_seq, attention_pred_seq = self.backtracking(
        ...
        accum_logP_seq)
      cur_fscore = self.get_fscore(label_pred_seq, label_true_seq,
                                   f_score_index_begin)

      # If t >= 2, compute the reward,
      # and generate the experience tuple ( s_{t-1}, a_{t-1}, r_{t-1}, s_t )
      if t >= 2:
        reward = self.get_reward(cur_fscore, fscore, cur_beam_size_in,
  ...
  def make_state(self, accum_logP_matrix, logP_matrix, beam_size,
                 max_beam_size):
    accum_logP_state, _ = \
    ...
    state = np.concatenate(
      (accum_logP_state, logP_state, np.array([beam_size])), axis=0)

    return state

  def backtracking(self, seq_len, batch_size, y_seq, beta_seq, attention_seq,
                   logP_seq, accum_logP_seq):
    # Only output the highest-scored beam (for each instance in the batch)
  ...
  def get_reward(self, cur_fscore, prev_fscore, cur_beam_size_in,
                 prev_beam_size_in, reward_coef_fscore, reward_coef_beam_size):
    reward = reward_coef_fscore * (
    cur_fscore - prev_fscore) * 0.01 + reward_coef_beam_size * (
    prev_beam_size_in - cur_beam_size_in)
    return reward
  ...
  def get_fscore(self, label_pred_seq_input, label_var_input,
                 f_score_index_begin):
    if self.gpu:
    ...
    label_pred_seq_padded = np.pad(label_pred_seq, (
    (0, 0), (0, label_var.shape[1] - label_pred_seq.shape[1])), "constant",
                                   constant_values=(f_score_index_begin - 1,
                                                    f_score_index_begin - 1))
  ...
  def evaluate_rl(self, eval_data_X, eval_data_Y, index2word, index2label,
    ...
    for batch_idx in range(batch_num):
      ...
      if decode_method == "greedy":
        ...
        attention_pred_seq, episode = self.decode_beam_adaptive_rl(
          current_sen_len, init_dec_hidden, init_dec_cell, enc_hidden_seq,
          beam_size, max_beam_size, agent, reward_coef_fscore,
          reward_coef_beam_size, label_var, f_score_index_begin)
        ### Debugging...
  ...
  def decode_beam_adaptive_rl(self, seq_len, init_dec_hidden, init_dec_cell,
                           enc_hidden_seq, initial_beam_size, max_beam_size,
                           agent, reward_coef_fscore, reward_coef_beam_size,
                           label_true_seq, f_score_index_begin):
    # Currently, batch size can only be 1
    ...
    for t in range(1, seq_len):
      ...
      accum_logP_matrix, logP_matrix, dec_hidden_beam, dec_cell_beam, attention_beam, accum_logP_output_beam, logP_output_beam = \
        self.decode_beam_step_rl(beam_size, y_beam, beta_beam,
                              dec_hidden_beam, dec_cell_beam, accum_logP_beam,
                              enc_hidden_seq, seq_len, t)
  ...
  def decode_beam_step_rl(self, beam_size_in, y_beam_in, beta_beam_in,
                       dec_hidden_beam_in, dec_cell_beam_in, accum_logP_beam_in,
                       enc_hidden_seq, seq_len, attend_index):
