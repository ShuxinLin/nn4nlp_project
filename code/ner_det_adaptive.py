...
class ner(nn.Module):
  ...
  def train(self, shuffle, beam_size, result_path):
    ...
    for epoch in range(self.max_epoch):
      ...
      # train_loss, train_fscore = self.evaluate(self.train_X, self.train_Y,
      #                                          None, None, "train", None,
      #                                          beam_size, decode_type='det_adaptive')
      # # Do evaluation on validation set as well
      # val_loss, val_fscore = self.evaluate(self.val_X, self.val_Y, None,
      #                                      None, "val", None, beam_size, decode_type='det_adaptive')
      # test_loss, test_fscore = self.evaluate(self.test_X, self.test_Y, None,
      #                                        None, "test", None, beam_size, decode_type='det_adaptive')
      #
      # print("epoch", epoch,
  ...
  def decode_det_adaptive(self, batch_size, sen_len,
                          init_dec_hidden, init_dec_cell, enc_hidden_seq,
                          beam_size):
    """
    ...
    Args:
      ...
      init_dec_cell: 
      enc_hidden_seq: 
      beam_size: initial beam size    

    Returns:

    """

    print("\nDecode adaptive called\n")
    ...
    if self.attention:
      ...
      attention = attention.view(batch_size, sen_len)
    ...
    # accumulated_logP_out_list = []

    # For output: (1, batch size, label dim)
    logP_out_beam = torch.stack([score_out], dim = 0)
    # Swap into shape (batch size, 1, label dim)
    score_output_beam = logP_out_beam.permute(1, 0, 2)  #switch B to idx=1

    # score_matrix.shape => (batch size, |V^y| * 1)
    # * 1 because there is only 1 input beam
    score_matrix = torch.cat([score_out], dim = 1)  # concat in a row
    ...
    logP_beam, index_beam = torch.topk(score_matrix, beam_size, dim = 1)
    ...
    dec_hidden_beam_out = dec_hidden_beam
    dec_cell_beam_out = dec_cell_beam
    for t in range(1, sen_len):
      ...
      accum_logP_matrix, logP_matrix, dec_hidden_beam_out, \
      dec_cell_beam_out = \
        self.decode_det_adaptive_beam_step(beam_size_in=beam_size,
                                    y_beam_in=y_beam,
                                    beta_beam_in=beta_beam,
                                    dec_hidden_beam_in=dec_hidden_beam_out,
                                    dec_cell_beam_in=dec_cell_beam_out,
                                    accum_logP_beam_in=logP_beam,
                                    enc_hidden_seq=enc_hidden_seq,
                                    attend_index=t)
    ...
    label_pred_seq = y_seq[sen_len - 1][:, 0].contiguous() \
      .view(batch_size, 1)
    input_beam = beta_seq[sen_len - 1][:, 0].view(batch_size, 1)
    ...
    if self.attention:
      ...
      attention_pred_seq = (attention_seq[sen_len - 1][range(batch_size),
                            input_beam, :])[None, :, :]

    # import pdb; pdb.set_trace()
    score_pred_seq = \
      (score_seq[sen_len - 1][range(batch_size), input_beam, :])[None, :, :]

    for t in range(sen_len - 2, -1, -1):
      ...
      score_pred_seq = torch.cat(
        [(score_seq[t][range(batch_size), input_beam, :])[None, :, :],
         score_pred_seq], dim=0)
    # End for t
    ...
    # a convenient shape (batch_size * seq_len, label_size)
    # for later cross entropy loss
    score_pred_seq = score_pred_seq.view(batch_size * sen_len, self.label_size)
  ...
  def evaluate(self, eval_data_X, eval_data_Y, index2word, index2label,
               suffix, result_path, beam_size, decode_type='greedy'):
    ...
    for batch_idx in range(batch_num):
      ...
      elif decode_type == 'det_adaptive':
        label_pred_seq, score_pred_seq, attention_pred_seq = \
          self.decode_det_adaptive(current_batch_size, current_sen_len,
                                   init_dec_hidden, init_dec_cell,
  ...
  def decode_det_adaptive_beam_step(self, beam_size_in, y_beam_in, beta_beam_in,
                       dec_hidden_beam_in, dec_cell_beam_in, accum_logP_beam_in,
                       enc_hidden_seq, attend_index):
    """
    
    Args:
      beam_size_in: 
      y_beam_in: label index (B x Beam =  1 x Beam), B is always 1   
      beta_beam_in: input beam index, B x Beam = 1 x Beam  
      dec_hidden_beam_in: Beam x B x H
      dec_cell_beam_in: Beam x B x H 
      accum_logP_beam_in: 
      enc_hidden_seq: 
      attend_index: 
