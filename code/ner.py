...
class ner(nn.Module):
  def __init__(self,
               ...
               test_X=None, test_Y=None, attention='bahdanau'):
  ...
  def decode_train(self, label_seq, init_dec_hidden, init_dec_cell,
                   enc_hidden_seq):  # added for attention
    ...
    init_label_emb = self.label_embedding(
      Variable(torch.LongTensor(self.minibatch_size,1).zero_() +
               LABEL_BEGIN_INDEX)).view(self.minibatch_size,
                                        self.label_embedding_dim)
    ...
    return dec_hidden_seq, score_seq, attention
  ...
  def train(self):
    ...
    for epoch in range(self.max_epoch):
      ...
      for batch_idx in range(batch_num):
        ...
        init_dec_cell = enc_cell_out[-1]

        # Attention added
        dec_hidden_seq, score_seq, attention = \
  ...
  def decode_greedy(self, seq_len, init_dec_hidden, init_dec_cell,
    ...
    return label_pred_seq, attention
  ...
  def test(self):
    ...
    for batch_idx in range(batch_num):
      ...
      # change to the last element
      init_dec_hidden = enc_hidden_out[-1]
      init_dec_cell = enc_cell_out[-1]

      label_pred_seq, attention = \
        self.decode_greedy(current_sen_len, init_dec_hidden,
                           init_dec_cell, enc_hidden_seq)
  ...
  def eval_on_train(self):
    ...
    f_result_processed_train = open(result_path + "result_processed_train.txt",
                                    'w')
    ...
    for batch_idx in range(batch_num):
      ...
      label_pred_seq, attention = self.decode_greedy(current_sen_len,
                                                     init_dec_hidden,
                                                     init_dec_cell,
                                                     enc_hidden_seq)
